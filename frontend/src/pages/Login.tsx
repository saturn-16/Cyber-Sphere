...
export default function Login() {
  const { loginWithGoogle, loginWithGithub, loginWithRememberedUser, isAuthenticated, isLoading } = useAuthStore();
  const navigate = useNavigate();
  const [error, setError] = useState<string | null>(null);
  const [rememberedUser, setRememberedUser] = useState<any>(null);

  useEffect(() => {
    const cached = localStorage.getItem('cybersphere_remembered_operator');
    if (cached) {
      try {
        setRememberedUser(JSON.parse(cached));
      } catch (e) {
        console.error('Failed to parse cached operator', e);
      }
    }
  }, []);
          ...
          <div className="pt-4">
            {rememberedUser ? (
              <div className="space-y-6 text-center font-mono">
                ...
                <div className="flex flex-col items-center p-6 rounded-xl border border-white/5 bg-white/5 backdrop-blur-md relative overflow-hidden group">
                  {/* Subtle profile border glow */}
                  <div className="absolute inset-0 border border-cyber-cyan/15 rounded-xl pointer-events-none group-hover:border-cyber-cyan/35 transition-colors duration-300" />
                  {rememberedUser.photoURL ? (
                    <img
                      src={rememberedUser.photoURL}
                      alt={rememberedUser.displayName}
                      className="w-16 h-16 rounded-full border border-cyber-cyan/30 shadow-lg shadow-cyber-cyan/10"
                    />
                  ) : (
                    <div className="w-16 h-16 rounded-full border border-cyber-cyan/30 bg-cyber-cyan/10 flex items-center justify-center text-cyber-cyan font-bold text-xl">
                      {rememberedUser.displayName?.charAt(0) || 'O'}
                    </div>
                  )}
                  <h3 className="mt-4 text-base font-bold text-white tracking-tight leading-none">
                    {rememberedUser.displayName}
                  </h3>
                  <p className="text-[10px] text-zinc-400 mt-1.5">{rememberedUser.email}</p>
                  
                  <div className="mt-3.5 text-[8px] text-[#00f5ff] uppercase tracking-[0.15em] px-2.5 py-1 rounded bg-[#00f5ff]/5 border border-[#00f5ff]/10">
                    Remembered Profile
                  </div>
                </div>

                {/* Continue and switch controls */}
                <div className="space-y-3">
                  <button
                    onClick={() =>
                      loginWithRememberedUser(rememberedUser).catch((err) =>
                        setError(err.message || 'Authentication failed')
                      )
                    }
                    disabled={isLoading}
                    className="w-full flex items-center justify-center space-x-2 px-5 py-3.5 bg-gradient-to-r from-cyber-cyan/20 to-[#00f5ff]/10 hover:from-cyber-cyan/30 hover:to-[#00f5ff]/20 border border-cyber-cyan/40 hover:border-cyber-cyan/60 rounded-xl transition-all duration-300 shadow-lg shadow-cyber-cyan/10 active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none"
                  >
                    <span className="text-xs font-bold tracking-wider text-cyber-cyan font-mono uppercase">
                      {isLoading ? 'Connecting...' : `Continue as ${rememberedUser.displayName?.split(' ')[0]}`}
                    </span>
                  </button>
                  
                  <button
                    onClick={() => {
                      localStorage.removeItem('cybersphere_remembered_operator');
                      setRememberedUser(null);
                      setError(null);
                    }}
                    disabled={isLoading}
                    className="text-[10px] text-zinc-500 hover:text-zinc-300 transition-colors uppercase tracking-[0.1em] hover:underline"
                  >
